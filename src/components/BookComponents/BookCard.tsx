...
const BookCard = (props: Book) => {
  ...
  return (
    ...
    >
      ...
      {props.subscriptionRequired && !subscription && (
        <div className="absolute right-0 top-0 flex h-[18px] w-fit items-center rounded-[20px] bg-primary px-2 py-0 text-[10px] text-white">
          Premium
        </div>
      )}
      ...
      <div className="mb-2 text-sm text-[#394547]">{props.subTitle}</div>
      <div className="flex gap-2">
        <div className="flex items-center gap-1 text-sm font-light text-[#6b757b]">
          <div className="flex h-4 w-4">
            <Image src={clockIcon} alt="" />
          </div>
          <div className="text-sm font-light text-[#6b757b]">{formatTime(duration)}</div>
        </div>
        <div className="flex items-center gap-1 text-sm font-light text-[#6b757b]">
          <div className="flex h-4 w-4">
